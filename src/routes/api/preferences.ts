    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          const { genres, tropes, spiceLevel, pacing } = body as UserPreferences

          // Validate inputs
          if (!Array.isArray(genres) || genres.length === 0) {
            return json({ error: 'At least one genre is required' }, { status: 400 })
          }

          if (!Array.isArray(tropes) || tropes.length === 0) {
            return json({ error: 'At least one trope is required' }, { status: 400 })
          }

          // Validate genres are valid
          const invalidGenres = genres.filter((g) => !GENRES.includes(g as any))
          if (invalidGenres.length > 0) {
            return json({ error: 'Invalid genre selection' }, { status: 400 })
          }

          // Validate tropes are valid
          const invalidTropes = tropes.filter((t) => !TROPES.includes(t as any))
          ...
          return json({ success: true, preferences: { genres, tropes, spiceLevel, pacing } })
        } catch (error) {
